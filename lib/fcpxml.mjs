...
// FCPXML caption on lane="-1" below the spine. `format` is the FCP
// caption-role suffix (ITT / SRT / 608). Cue timestamps are in seconds.
// All opts fields are required.
export function caption(opts) {
  if (!opts.rateNum || !opts.rateDen) throw new Error("caption: rateNum + rateDen required");
  if (opts.id == null || opts.text == null) throw new Error("caption: id + text required");
  if (opts.offsetSec == null || opts.durSec == null) throw new Error("caption: offsetSec + durSec required");
  if (!opts.lang || !opts.format || !opts.name) throw new Error("caption: name + lang + format required");
  const fps = opts.rateNum / opts.rateDen;
  const offsetFrames = Math.round(opts.offsetSec * fps);
  const durFrames = Math.max(1, Math.round(opts.durSec * fps));
  const role = `iTT?captionFormat=${opts.format}.${opts.lang}`;
  const tsId = `ts${opts.id}`;
  return `<caption lane="-1" offset="${rt(offsetFrames, opts.rateNum, opts.rateDen)}" name="${esc(opts.name)}" start="0s" duration="${rt(durFrames, opts.rateNum, opts.rateDen)}" role="${role}"><text><text-style ref="${tsId}">${esc(opts.text)}</text-style></text><text-style-def id="${tsId}"><text-style font=".SF NS" fontSize="13" fontColor="1 1 1 1" backgroundColor="0 0 0 1" alignment="center"/></text-style-def></caption>`;
}

// Emit one `<caption>` per cue. cues = [{ startSec, endSec, text }, ...].
export function emitCaptions(cues, opts) {
  if (!opts.rateNum || !opts.rateDen || !opts.lang || !opts.format) {
    throw new Error("emitCaptions: rateNum + rateDen + lang + format required");
  }
  return cues.map((cue, i) => caption({
    id: i + 1,
    name: `Caption ${i + 1}`,
    offsetSec: cue.startSec,
    durSec: Math.max(0.1, cue.endSec - cue.startSec),
    text: cue.text,
    lang: opts.lang, format: opts.format,
    rateNum: opts.rateNum, rateDen: opts.rateDen,
  })).join("");
}
