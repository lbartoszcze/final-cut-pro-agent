...
import { stageLogo, stageFreeze, stageVO, stageSFX, stageRetime, stageMusic, stageCodec, stageColorspace, CODEC_MAP } from "../cli/post.mjs";
...
const stages = ["titles", capOn ? "captions" : null, pipOn ? "pip" : null, logoOn ? "logo" : null, sfxOn ? "sfx" : null, args.freeze ? "freeze" : null, voOn ? "vo" : null, speed !== 1 ? "retime" : null, args.music ? "music" : null, transcodeOn ? "codec" : null, args.colorspace ? "cs" : null].filter(Boolean);
...
overlayTitles(concatPath, plan.titles, plan.totalSec, pathFor("titles"), aspect);
...
if (transcodeOn) { console.log(`  [6] codec → ${codecSpec}${args.bitrate ? ` @ ${args.bitrate}` : ""}`); stageCodec(acc, codecSpec, pathFor("codec"), args.bitrate); acc = pathFor("codec"); }
if (args.colorspace) { console.log(`  [7] color-space → ${args.colorspace}`); stageColorspace(acc, args.colorspace, pathFor("cs")); }
