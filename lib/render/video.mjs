...
import { ffmpeg, ffprobeDurationSec, renderClips, overlayTitles, renderColor } from "./ffmpeg.mjs";
...
function parseArgs(argv) {
  const out = { mode: "test-pattern", style: "montage", bpm: "140", bars: "16", clips: "", out: join(ROOT, "cut-rendered.mp4") };
  for (const a of argv) {
    const m = a.match(/^--([^=]+)=(.+)$/);
    if (m) out[m[1]] = m[2];
  }
...
const plan = build(args.style, bars, bpm, { clipDurations, projectName, seed: (bpm * 1000 + bars) >>> 0 });
...
console.log("  [1/2] concatenating clips with cuts + section fades...");
renderClips(clipPaths, plan.cuts, plan.transitions, plan.totalSec, concatPath);
