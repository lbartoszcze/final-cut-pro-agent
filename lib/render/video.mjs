...
import { ffmpeg, ffprobeDurationSec, renderClips, overlayTitles, renderColor, probeLoudness } from "./ffmpeg.mjs";
...
function parseArgs(argv) {
  const out = { mode: "test-pattern", style: "montage", bpm: "140", bars: "16", clips: "", out: join(ROOT, "cut-rendered.mp4"), template: "", look: "cinematic", "audio-target": "-16", "audio-fade": "0.05" };
  for (const a of argv) {
    const m = a.match(/^--([^=]+)=(.+)$/);
    if (m) out[m[1]] = m[2];
  }
...
console.log("  [1/2] concatenating clips with cuts + grade + audio normalization...");
renderClips(clipPaths, plan.cuts, plan.transitions, plan.totalSec, concatPath, look.ffmpeg, audioOpts);
