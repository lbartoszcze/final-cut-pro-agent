...
import { ffmpeg, ffprobeDurationSec, renderClips, overlayTitles, renderColor, probeLoudness, parseAspect } from "./ffmpeg.mjs";
import { parseTemplate, applyTemplate } from "./template.mjs";
import { resolveLook } from "./grades.mjs";
...
function parseArgs(argv) {
  const out = { mode: "test-pattern", style: "montage", bpm: "140", bars: "16", clips: "", out: join(ROOT, "cut-rendered.mp4"), template: "", look: "cinematic", "audio-target": "-16", "audio-fade": "0.05", aspect: "16:9" };
  for (const a of argv) {
    const m = a.match(/^--([^=]+)=(.+)$/);
    if (m) out[m[1]] = m[2];
  }
...
console.log(`  output aspect: ${aspect.w}x${aspect.h} (${aspect.mode}-fit) [from --aspect=${args.aspect}]`);

console.log("  [1/2] concatenating clips with cuts + grade + audio + aspect reframe...");
renderClips(clipPaths, plan.cuts, plan.transitions, plan.totalSec, concatPath, look.ffmpeg, audioOpts, aspect);
