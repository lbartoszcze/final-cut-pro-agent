...
function parseArgs(argv) {
  const out = { mode: "test-pattern", style: "montage", bpm: "140", bars: "16", clips: "", out: join(ROOT, "cut-rendered.mp4"), template: "", look: "cinematic" };
  for (const a of argv) {
    const m = a.match(/^--([^=]+)=(.+)$/);
    if (m) out[m[1]] = m[2];
  }
...
console.log("  [1/2] concatenating clips with cuts + grade + section fades...");
renderClips(clipPaths, plan.cuts, plan.transitions, plan.totalSec, concatPath, look.ffmpeg);
