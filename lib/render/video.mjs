...
import { ffmpeg, ffprobeDurationSec, renderClips, overlayTitles, renderColor, probeLoudness, parseAspect, parseFps } from "./ffmpeg.mjs";
...
function parseArgs(argv) {
  const out = { mode: "test-pattern", style: "montage", bpm: "140", bars: "16", clips: "", out: join(ROOT, "cut-rendered.mp4"), template: "", look: "cinematic", "audio-target": "-16", "audio-fade": "0.05", aspect: "16:9", fps: "29.97", "fade-from-black": "0", "fade-to-black": "0", "max-duration": "" };
  for (const a of argv) {
    const m = a.match(/^--([^=]+)=(.+)$/);
    if (m) out[m[1]] = m[2];
  }
...
renderClips(clipPaths, effectiveCuts, plan.transitions, effectiveTotal, concatPath, look.ffmpeg, audioOpts, aspect, fps, { fadeFromBlackSec: fadeFromBlack, fadeToBlackSec: fadeToBlack });
