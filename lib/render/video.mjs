...
const stages = ["titles", capOn ? "captions" : null, pipOn ? "pip" : null, logoOn ? "logo" : null, sfxOn ? "sfx" : null, args.freeze ? "freeze" : null, voOn ? "vo" : null, speed !== 1 ? "retime" : null, args.music ? "music" : null, args.colorspace ? "cs" : null, transcodeOn ? "codec" : null].filter(Boolean);
...
if (transcodeOn) { console.log(`  [7] codec → ${codecSpec}${args.bitrate ? ` @ ${args.bitrate}` : ""}`); stageCodec(acc, codecSpec, pathFor("codec"), args.bitrate); }
