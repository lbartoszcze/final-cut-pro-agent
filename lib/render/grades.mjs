...
// Cosmetic post-processing filters: vignette (corner darkening), film grain
// (analog texture), sharpening. Each maps a 0-to-N intensity to an ffmpeg
// filter chain; intensity = 0 returns empty so callers can skip.
export function vignetteFilter(intensity) {
  const v = parseFloat(intensity) || 0;
  if (v <= 0) return "";
  // PI/4 = 45 deg field-of-view = strong; PI/2 = no vignette. Map 0-1 to PI/2..PI/5.
  const angle = Math.PI / 2 - (Math.PI / 2 - Math.PI / 5) * Math.min(1, v);
  return `vignette=angle=${angle.toFixed(4)}`;
}
export function grainFilter(amount) {
  const v = parseFloat(amount) || 0;
  if (v <= 0) return "";
  // ffmpeg noise filter: alls=N where N is 0-100 strength, allf=t means
  // temporal noise (non-static — looks like real grain).
  return `noise=alls=${Math.round(Math.min(100, v))}:allf=t`;
}
export function sharpenFilter(amount) {
  const v = parseFloat(amount) || 0;
  if (v <= 0) return "";
  // unsharp luma_msize_x:luma_msize_y:luma_amount — amount in [-1.5, 1.5].
  const a = Math.min(1.5, Math.max(0, v));
  return `unsharp=5:5:${a.toFixed(2)}:5:5:0.0`;
}
