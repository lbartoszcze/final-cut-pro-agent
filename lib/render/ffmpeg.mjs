...
// ffmpeg audio chain: optional denoise + loudnorm + optional limiter + edge fades.
...
export function renderClips(clipPaths, cuts, transitions, totalSec, outPath, gradeFilter, audioOpts, aspect, fps, opts) {
  ...
  cuts.forEach((c, i) => filters.push(`[${c.srcIdx}:v]trim=start=${c.srcInSec.toFixed(3)}:duration=${c.durSec.toFixed(3)},setpts=PTS-STARTPTS,${c.vFilter ? c.vFilter + "," : ""}${fitChain},fps=${fr.label}${grade}[v${i}]`));
  ...
  for (let k = 1; k < segs.length; k++) {
    ...
    filters.push(`[${aV}][${segs[k].vLabel}]xfade=transition=fade:duration=${d.toFixed(3)}:offset=${Math.max(0, aDur - d).toFixed(3)}[xv${k}]`);
    aV = `xv${k}`;
...
export function overlayTitles(srcPath, titles, totalSec, outPath, aspect) {
  if (titles.length === 0) {
  ...
  const styleFor = (pos) => pos === "lower-third"
    ? { sz: vertical ? 48 : 64, x: "w*0.05", y: vertical ? "h*0.72" : "h-h*0.18", box: "1", bc: "black@0.6", bw: "16" }
    : pos === "end-card"
    ? { sz: vertical ? 96 : 130, x: "(w-text_w)/2", y: "(h-text_h)/2", box: "0", bc: "black@0", bw: "0" }
    : { sz: vertical ? 72 : 96, x: "(w-text_w)/2", y: "(h-text_h)/2", box: "1", bc: "black@0.4", bw: "20" };
  const drawtextFilters = titles.map((t) => {
    const s = styleFor(t.position);
    ...
    return `drawtext=text='${escapeText(t.text)}':fontcolor=white:fontsize=${s.sz}:fontfile=/System/Library/Fonts/Helvetica.ttc:x=${s.x}:y=${s.y}:enable='${enable}':alpha='${alpha}':box=${s.box}:boxcolor=${s.bc}:boxborderw=${s.bw}`;
  });
