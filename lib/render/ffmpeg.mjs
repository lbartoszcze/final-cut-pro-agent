...
export function renderClips(clipPaths, cuts, transitions, totalSec, outPath, gradeFilter = "", audioOpts = null, aspect = null, fps = null) {
  ...
  cuts.forEach((c, i) => {
    ...
    filters.push(`[${c.srcIdx}:v]trim=start=${c.srcInSec.toFixed(3)}:duration=${c.durSec.toFixed(3)},setpts=PTS-STARTPTS,${fitChain},fps=${fr.label}${grade}${fadeIn}${fadeOut}[v${i}]`);
  });
  if (audioOpts) {
    cuts.forEach((c, i) => {
