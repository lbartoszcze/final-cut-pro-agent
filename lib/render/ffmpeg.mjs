...
export function renderClips(clipPaths, cuts, transitions, totalSec, outPath, gradeFilter = "", audioOpts = null, aspect = null) {
  const fit = aspect || { w: 1920, h: 1080, mode: "fit" };
  if (cuts.length === 0) {
    return ffmpeg(["-f", "lavfi", "-i", `color=c=black:s=${fit.w}x${fit.h}:r=30000/1001:d=${totalSec.toFixed(3)}`, "-c:v", "libx264", "-pix_fmt", "yuv420p", outPath]);
  }
  ...
  cuts.forEach((c, i) => {
    ...
    filters.push(`[${c.srcIdx}:v]trim=start=${c.srcInSec.toFixed(3)}:duration=${c.durSec.toFixed(3)},setpts=PTS-STARTPTS,${fitChain},fps=30000/1001${grade}${fadeIn}${fadeOut}[v${i}]`);
  });
  if (audioOpts) {
    cuts.forEach((c, i) => {
