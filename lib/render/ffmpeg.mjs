...
// transitions: array of { tOnTimeline, durSec, kind: 'xfade' }
// gradeFilter: optional ffmpeg filter chain (e.g. from grades.mjs LOOKS) spliced
// into each per-clip pipeline so the color look applies before concatenation.
export function renderClips(clipPaths, cuts, transitions, totalSec, outPath, gradeFilter = "") {
  if (cuts.length === 0) {
