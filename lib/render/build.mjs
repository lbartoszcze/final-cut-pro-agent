...
// Lens flare / light leak: a soft additive blob travels across the frame.
// intensity 0..1; ~0.3 is subtle, 0.7 is dramatic.
function lensFlareFilter(intensity, durSec) {
  if (!Number.isFinite(intensity) || intensity <= 0) return "";
  const g = Math.min(1, Math.max(0, intensity)) * 0.5;
  return `geq=r='r(X,Y)+${(255 * g).toFixed(1)}*exp(-pow((X-(W*0.2+W*0.6*T/${durSec.toFixed(3)})),2)/pow(W*0.1,2)-pow((Y-H/2),2)/pow(H*0.2,2))':g='g(X,Y)+${(220 * g).toFixed(1)}*exp(-pow((X-(W*0.2+W*0.6*T/${durSec.toFixed(3)})),2)/pow(W*0.1,2)-pow((Y-H/2),2)/pow(H*0.2,2))':b='b(X,Y)+${(180 * g).toFixed(1)}*exp(-pow((X-(W*0.2+W*0.6*T/${durSec.toFixed(3)})),2)/pow(W*0.1,2)-pow((Y-H/2),2)/pow(H*0.2,2))'`;
}

// Lens correction (barrel / pincushion undistort). strength: 0..1 typical.
function lensCorrectFilter(spec) {
  if (!spec) return "";
  const m = spec.match(/^(barrel|pincushion)(?::(-?[\d.]+))?$/);
  if (!m) throw new Error(`bad --lens-correct: ${spec}`);
  const strength = m[2] ? parseFloat(m[2]) : 0.3;
  const k1 = m[1] === "barrel" ? -strength : strength;
  return `lenscorrection=k1=${k1.toFixed(3)}:k2=0`;
}
...
export function build(style, bars, bpm, opts) {
  ...
  for (let bar = 0; bar < bars; bar++) {
    ...
    for (let i = 0; i < cutPlan.length; i++) {
      ...
      const parts = [];
      if (opts.lensCorrect) {
        const lc = lensCorrectFilter(opts.lensCorrect);
        if (lc) parts.push(lc);
      }
      if (opts.chromaKey) {
      ...
      if (opts.crop) {
        // "--crop=w:h:x:y" — ffmpeg crop filter (literal pass-through).
        if (!/^[\d:]+$/.test(opts.crop)) throw new Error(`bad --crop: ${opts.crop} (expect w:h:x:y all numeric)`);
        parts.push(`crop=${opts.crop}`);
      }
      if (opts.lensFlare > 0) {
        const lf = lensFlareFilter(opts.lensFlare, durSec);
        if (lf) parts.push(lf);
      }
