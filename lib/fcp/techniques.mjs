...
export const TECHNIQUES = {
  ...
  "caption-edit":          () => { need(); clickMenu(["Edit", "Captions", "Edit Caption"]); console.log("edit caption"); },
  "caption-split":         () => { need(); clickMenu(["Edit", "Captions", "Split Captions"]); console.log("split captions"); },
  "caption-resolve":       () => { need(); clickMenu(["Edit", "Captions", "Resolve Overlaps"]); console.log("resolve overlaps"); },
  "caption-extract":       () => { need(); clickMenu(["Edit", "Captions", "Extract Captions"]); console.log("extract captions"); },
  "caption-transcribe":    () => {
    need();
  ...
  // Duplicate captions to a new language by name (e.g. "Spanish", "French",
  // "Japanese"). Drives Edit > Captions > Duplicate Captions to New Language > <lang>.
  "caption-duplicate-language"([lang]) {
    if (!lang) throw new Error("caption-duplicate-language <Spanish|French|...>");
    need();
    clickMenu(["Edit", "Captions", "Duplicate Captions to New Language", lang]);
    console.log(`caption duplicate -> ${lang}`);
  },
  // Duplicate captions to a different format (iTT, CEA-608, SRT).
  "caption-duplicate-format"([fmt]) {
    if (!fmt) throw new Error("caption-duplicate-format <iTT|CEA-608|SRT>");
    need();
    clickMenu(["Edit", "Captions", "Duplicate Captions to New Format", fmt]);
    console.log(`caption duplicate -> format ${fmt}`);
  },
...
export const TECHNIQUES_HELP = [
  ...
  ["Playback",          ["scopes"]],
  ["Captions",          ["caption-add", "caption-edit", "caption-split", "caption-resolve", "caption-extract", "caption-transcribe", "caption-duplicate-language <lang>", "caption-duplicate-format <fmt>"]],
