...
// Scan a folder of music tracks, detect each one's tempo, and return the
// best match for the target duration `targetSec`. Score = inverse of how far
// each track's natural duration (at its detected BPM, given the supplied
// `bars`) is from the target. Returns { path, bpm, durationSec, confidence }
// or null when the folder is empty / unscan-able.
import { readdirSync, statSync } from "node:fs";
import { join as pathJoin, extname } from "node:path";

export function autoPickMusic(folder, targetSec, bars) {
  const AUDIO_EXT = new Set([".mp3", ".wav", ".m4a", ".aac", ".flac", ".ogg"]);
  let files;
  try {
    files = readdirSync(folder).filter((n) => AUDIO_EXT.has(extname(n).toLowerCase())).map((n) => pathJoin(folder, n));
  } catch (e) { return null; }
  if (files.length === 0) return null;
  let best = null;
  for (const p of files) {
    try {
      const t = detectTempo(p);
      const naturalSec = (60 / t.bpm) * 4 * bars;
      const diff = Math.abs(naturalSec - targetSec);
      const score = -diff;
      if (!best || score > best.score) best = { path: p, bpm: t.bpm, durationSec: naturalSec, confidence: t.confidence, score };
    } catch (_) {}
  }
  return best;
}
...
// Cross-correlate two audio tracks to find the time offset that aligns
// them — used for multicam sync when filenames don't match. We decode each
// to mono 11025 Hz PCM (kick-band only — sharp transients give the cleanest
// correlation peak) and slide track B against track A over ±maxShiftSec.
// Returns the offset (seconds) to add to B's clock to align with A.
export function audioSyncOffset(pathA, pathB, maxShiftSec = 10) {
  const a = decodeKickBand(pathA);
  const b = decodeKickBand(pathB);
  const oA = onsetEnvelope(a);
  const oB = onsetEnvelope(b);
  const hopSec = HOP / SR;
  const maxLag = Math.min(Math.round(maxShiftSec / hopSec), Math.min(oA.length, oB.length) - 1);
  let bestLag = 0, bestScore = -Infinity;
  for (let lag = -maxLag; lag <= maxLag; lag++) {
    let sum = 0, n = 0;
    for (let i = 0; i < oA.length; i++) {
      const j = i + lag;
      if (j < 0 || j >= oB.length) continue;
      sum += oA[i] * oB[j];
      n++;
    }
  ...
  return { offsetSec: bestLag * hopSec, confidence: bestScore };
}
